...
- **Signposts** (fully procedural, `CreateSignpost` — code-built plate + text, no imported model): hospital entrance ("AIZAWL DISTRICT HOSPITAL / EMERGENCY SERVICES"), shelter entrance ("COMMUNITY EVACUATION SHELTER / DISTRICT RELIEF HQ"), landslide warning ("CAUTION: LANDSLIDE HAZARD / ROAD CLOSED - EVACUATE TO HIGH GROUND"), town welcome sign ("AIZAWL HILL DISTRICT / ELEVATION 1132m - EVAC ROUTE NORTH"), central junction wayfinding cluster added in the autonomous polish pass at `(3, 2.99, Z=-9/-7/-5)` ("HOSPITAL / WEST AT JUNCTION", "EMERGENCY SHELTER / EAST AT JUNCTION", "SUMMIT / HIGHEST POINT / CONTINUE NORTH") -- placed on the east shoulder of the central junction so all three route choices are legible right at the decision point, before this the only signage was at each destination's own entrance.
